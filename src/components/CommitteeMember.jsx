...
export default function CommitteeMember({ number, name, position, quote, image, flip, socials }) {
  ...
  const content = (
    <div className="flex items-center justify-center md:flex-row flex-col">
      <div
        className={`
          w-[80vw] max-w-[300px] rounded-full aspect-square overflow-hidden
          md:w-[50%] md:max-w-none
          ${flip ? 'md:order-2' : 'order-1'}
          relative
          shadow-[0_0_15px_5px_rgba(0,0,0,0.6)]
        `}
      >
        <div className="rounded-full overflow-hidden w-full h-full bg-white relative">
          <img
            src={image}
            alt={`${position.toLowerCase()}-photo`}
      ...
      <div
        className={`
          text-lg text-slate-800 p-4 w-full md:w-[50%] text-center md:text-left
          ${flip ? 'order-1' : 'order-2'} flex flex-col items-center
        `}
      >
